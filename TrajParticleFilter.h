  ...
  class TrajParticleFilter {
    public:
      TrajParticleFilter(const Model* m) 
        : model(m), curStep(0), curLin(1), pfpt(0),
      ...
      TrajParticle& operator[](size_t i) { return pop()[i]; }
      const Particle& at(size_t i) const { return pop().at(i); }
    ...
    protected:
      ...
      inline vector<TrajParticle>& pop() { return pf[getCurStep()]; }
      inline const vector<TrajParticle>& pop() const { return pf[getCurStep()]; }
